...
int DeviceFinding()
{
	...
	//}
	std::vector<const PvDeviceInfo*> lDeviceInfoList(cam_max_number);
...
*/
			...
			for (uint32_t y = 0; y < lDeviceCountNow; y++)
			{
				...
				{
					std::cout << "    MAC Address: " << lDeviceInfoPleora->GetMACAddress().GetAscii() << endl;
					std::cout << "    IP Address: " << lDeviceInfoPleora->GetIPAddress().GetAscii() << endl;
					std::cout << "    Serial number: " << lDeviceInfoPleora->GetSerialNumber().GetAscii() << endl << endl;
				}
				*/
				else
				{
